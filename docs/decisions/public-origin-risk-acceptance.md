...
| No edge WAF/identity layer | No edge identity gate, WAF, IP reputation, or edge access logs | Documented risk owner, monitoring, strict LiteLLM budgets/rate limits, and separate hardening design if public-origin risk becomes unacceptable |
...
| Tunnel/edge hardening | Removed from current scope. Do not configure or deploy Cloudflare Tunnel, Access, WAF, service tokens, or edge origin guards for this phase. |
...
Revisit IP allowlisting or a separately approved edge-origin hardening design if any of these happen:
